...
| SDW-103 资源和模式权限模型 | 站点能力已有，终端能力待实现 | 现有 Cloud 有租户、角色、Node Pool 和 Grant；Client Projection 的 policy/global 权限模型尚未建立 |
...
| SDW-107 终端设备注册和 Client Grant/Projection 接口 | 合同已冻结，Cloud 实现待推进 | `contracts/client_control.md`、`contracts/client_control.schema.json`、`contracts/examples/client_*.json`；已覆盖设备/密钥/Grant/Projection/回执绑定和幂等边界，尚未接入 `candy-cloud` 数据库和路由 |
